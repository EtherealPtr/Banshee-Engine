...
namespace Banshee
{
	...
	{
	public:
		Mesh(const std::string_view modelPath, const ShaderType shaderType = ShaderType::Standard) :
		...
		Mesh(const PrimitiveShape basicShape, const ShaderType shaderType = ShaderType::Standard) :
			MeshComponent{ basicShape, shaderType },
			m_ParentBufferId{ 0 },
			m_MaterialIndex{ SetNextMaterialIndex() },
			m_IndexOffset{ 0 },
			m_TexId{ 0 },
