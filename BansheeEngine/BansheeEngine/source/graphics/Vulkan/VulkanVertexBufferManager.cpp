...
namespace Banshee
{
	...
	void VulkanVertexBufferManager::GenerateBuffers(const uint32_t _bufferId, void* _vertexData, const uint64_t _sizeOfVertexData, void* _indexData, const uint64_t _sizeOfIndexData)
	{
	...
	void VulkanVertexBufferManager::Bind(const uint32_t _bufferId, const VkCommandBuffer& _commandBuffer)
	{
		...
		else
		{
			...
			GenerateBuffers(_bufferId, vertices.data(), sizeof(Vertex) * vertices.size(), indices.data(), sizeof(uint16_t) * indices.size());
