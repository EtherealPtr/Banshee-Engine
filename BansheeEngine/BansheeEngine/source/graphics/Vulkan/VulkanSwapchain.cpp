...
namespace Banshee
{
    ...
    VulkanSwapchain::VulkanSwapchain(const VkPhysicalDevice& _gpu, const VkDevice& _logicalDevice, const VkSurfaceKHR& _surface, const uint32 _w, const uint32 _h) :
        ...
        m_Height(0)
    {
        BE_LOG(LogCategory::trace, "Vulkan swapchain creation stage: Start");
        ...
        m_Format = static_cast<uint32>(surfaceFormat.format);
        ...
        {
            BE_LOG(LogCategory::error, "Failed to create a Vulkan swapchain");
            throw std::runtime_error("ERROR: Failed to create Vulkan swapchain\n");
        }

        BE_LOG(LogCategory::info, "Vulkan swapchain created successfully");
        BE_LOG(LogCategory::trace, "Retrieving swapchain images");
        ...
        BE_LOG(LogCategory::trace, "Creating image views for the swapchain images");
        m_SwapchainImageViews.resize(imageCount);

        for (uint32 i = 0; i < imageCount; ++i)
        {
            m_SwapchainImageViews[i] = VulkanUtils::CreateImageView(_logicalDevice, m_SwapchainImages[i], surfaceFormat.format);
        }

        assert(m_SwapchainImageViews.size() > 0);
        BE_LOG(LogCategory::trace, "Vulkan swapchain creation stage: End");
    }
    ...
    VulkanSwapchain::~VulkanSwapchain()
    {
        ...
        BE_LOG(LogCategory::trace, "Vulkan swapchain image view destroyed");

        vkDestroySwapchainKHR(m_Device, m_Swapchain, nullptr);
        m_Swapchain = VK_NULL_HANDLE;
        BE_LOG(LogCategory::trace, "Vulkan swapchain destroyed");
    }
