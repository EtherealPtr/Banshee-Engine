#pragma once

// Retrieved from GLFW version 3.3.8
#define SE_KEY_SPACE              32
#define SE_KEY_APOSTROPHE         39  /* ' */
#define SE_KEY_COMMA              44  /* , */
#define SE_KEY_MINUS              45  /* - */
#define SE_KEY_PERIOD             46  /* . */
#define SE_KEY_SLASH              47  /* / */
#define SE_KEY_0                  48
#define SE_KEY_1                  49
#define SE_KEY_2                  50
#define SE_KEY_3                  51
#define SE_KEY_4                  52
#define SE_KEY_5                  53
#define SE_KEY_6                  54
#define SE_KEY_7                  55
#define SE_KEY_8                  56
#define SE_KEY_9                  57
#define SE_KEY_SEMICOLON          59  /* ; */
#define SE_KEY_EQUAL              61  /* = */
#define SE_KEY_A                  65
#define SE_KEY_B                  66
#define SE_KEY_C                  67
#define SE_KEY_D                  68
#define SE_KEY_E                  69
#define SE_KEY_F                  70
#define SE_KEY_G                  71
#define SE_KEY_H                  72
#define SE_KEY_I                  73
#define SE_KEY_J                  74
#define SE_KEY_K                  75
#define SE_KEY_L                  76
#define SE_KEY_M                  77
#define SE_KEY_N                  78
#define SE_KEY_O                  79
#define SE_KEY_P                  80
#define SE_KEY_Q                  81
#define SE_KEY_R                  82
#define SE_KEY_S                  83
#define SE_KEY_T                  84
#define SE_KEY_U                  85
#define SE_KEY_V                  86
#define SE_KEY_W                  87
#define SE_KEY_X                  88
#define SE_KEY_Y                  89
#define SE_KEY_Z                  90
#define SE_KEY_LEFT_BRACKET       91  /* [ */
#define SE_KEY_BACKSLASH          92  /* \ */
#define SE_KEY_RIGHT_BRACKET      93  /* ] */
#define SE_KEY_GRAVE_ACCENT       96  /* ` */
#define SE_KEY_WORLD_1            161 /* non-US #1 */
#define SE_KEY_WORLD_2            162 /* non-US #2 */

// Function keys
#define SE_KEY_ESCAPE             256
#define SE_KEY_ENTER              257
#define SE_KEY_TAB                258
#define SE_KEY_BACKSPACE          259
#define SE_KEY_INSERT             260
#define SE_KEY_DELETE             261
#define SE_KEY_RIGHT              262
#define SE_KEY_LEFT               263
#define SE_KEY_DOWN               264
#define SE_KEY_UP                 265
#define SE_KEY_PAGE_UP            266
#define SE_KEY_PAGE_DOWN          267
#define SE_KEY_HOME               268
#define SE_KEY_END                269
#define SE_KEY_CAPS_LOCK          280
#define SE_KEY_SCROLL_LOCK        281
#define SE_KEY_NUM_LOCK           282
#define SE_KEY_PRINT_SCREEN       283
#define SE_KEY_PAUSE              284
#define SE_KEY_F1                 290
#define SE_KEY_F2                 291
#define SE_KEY_F3                 292
#define SE_KEY_F4                 293
#define SE_KEY_F5                 294
#define SE_KEY_F6                 295
#define SE_KEY_F7                 296
#define SE_KEY_F8                 297
#define SE_KEY_F9                 298
#define SE_KEY_F10                299
#define SE_KEY_F11                300
#define SE_KEY_F12                301
#define SE_KEY_F13                302
#define SE_KEY_F14                303
#define SE_KEY_F15                304
#define SE_KEY_F16                305
#define SE_KEY_F17                306
#define SE_KEY_F18                307
#define SE_KEY_F19                308
#define SE_KEY_F20                309
#define SE_KEY_F21                310
#define SE_KEY_F22                311
#define SE_KEY_F23                312
#define SE_KEY_F24                313
#define SE_KEY_F25                314
#define SE_KEY_KP_0               320
#define SE_KEY_KP_1               321
#define SE_KEY_KP_2               322
#define SE_KEY_KP_3               323
#define SE_KEY_KP_4               324
#define SE_KEY_KP_5               325
#define SE_KEY_KP_6               326
#define SE_KEY_KP_7               327
#define SE_KEY_KP_8               328
#define SE_KEY_KP_9               329
#define SE_KEY_KP_DECIMAL         330
#define SE_KEY_KP_DIVIDE          331
#define SE_KEY_KP_MULTIPLY        332
#define SE_KEY_KP_SUBTRACT        333
#define SE_KEY_KP_ADD             334
#define SE_KEY_KP_ENTER           335
#define SE_KEY_KP_EQUAL           336
#define SE_KEY_LEFT_SHIFT         340
#define SE_KEY_LEFT_CONTROL       341
#define SE_KEY_LEFT_ALT           342
#define SE_KEY_LEFT_SUPER         343
#define SE_KEY_RIGHT_SHIFT        344
#define SE_KEY_RIGHT_CONTROL      345
#define SE_KEY_RIGHT_ALT          346
#define SE_KEY_RIGHT_SUPER        347
#define SE_KEY_MENU               348

// Mouse buttons
#define SE_MOUSE_BUTTON_1         0
#define SE_MOUSE_BUTTON_2         1
#define SE_MOUSE_BUTTON_3         2
#define SE_MOUSE_BUTTON_4         3
#define SE_MOUSE_BUTTON_5         4
#define SE_MOUSE_BUTTON_6         5
#define SE_MOUSE_BUTTON_7         6
#define SE_MOUSE_BUTTON_8         7
#define SE_MOUSE_BUTTON_LAST      SE_MOUSE_BUTTON_8
#define SE_MOUSE_BUTTON_LEFT      SE_MOUSE_BUTTON_1
#define SE_MOUSE_BUTTON_RIGHT     SE_MOUSE_BUTTON_2
#define SE_MOUSE_BUTTON_MIDDLE    SE_MOUSE_BUTTON_3

// Joystick
#define SE_JOYSTICK_1             0
#define SE_JOYSTICK_2             1
#define SE_JOYSTICK_3             2
#define SE_JOYSTICK_4             3
#define SE_JOYSTICK_5             4
#define SE_JOYSTICK_6             5
#define SE_JOYSTICK_7             6
#define SE_JOYSTICK_8             7
#define SE_JOYSTICK_9             8
#define SE_JOYSTICK_10            9
#define SE_JOYSTICK_11            10
#define SE_JOYSTICK_12            11
#define SE_JOYSTICK_13            12
#define SE_JOYSTICK_14            13
#define SE_JOYSTICK_15            14
#define SE_JOYSTICK_16            15
#define SE_JOYSTICK_LAST          SE_JOYSTICK_16

// Gamepad
#define SE_GAMEPAD_BUTTON_A               0
#define SE_GAMEPAD_BUTTON_B               1
#define SE_GAMEPAD_BUTTON_X               2
#define SE_GAMEPAD_BUTTON_Y               3
#define SE_GAMEPAD_BUTTON_LEFT_BUMPER     4
#define SE_GAMEPAD_BUTTON_RIGHT_BUMPER    5
#define SE_GAMEPAD_BUTTON_BACK            6
#define SE_GAMEPAD_BUTTON_START           7
#define SE_GAMEPAD_BUTTON_GUIDE           8
#define SE_GAMEPAD_BUTTON_LEFT_THUMB      9
#define SE_GAMEPAD_BUTTON_RIGHT_THUMB     10
#define SE_GAMEPAD_BUTTON_DPAD_UP         11
#define SE_GAMEPAD_BUTTON_DPAD_RIGHT      12
#define SE_GAMEPAD_BUTTON_DPAD_DOWN       13
#define SE_GAMEPAD_BUTTON_DPAD_LEFT       14
#define SE_GAMEPAD_BUTTON_LAST            SE_GAMEPAD_BUTTON_DPAD_LEFT

#define SE_GAMEPAD_BUTTON_CROSS       SE_GAMEPAD_BUTTON_A
#define SE_GAMEPAD_BUTTON_CIRCLE      SE_GAMEPAD_BUTTON_B
#define SE_GAMEPAD_BUTTON_SQUARE      SE_GAMEPAD_BUTTON_X
#define SE_GAMEPAD_BUTTON_TRIANGLE    SE_GAMEPAD_BUTTON_Y

#define SE_GAMEPAD_AXIS_LEFT_X        0
#define SE_GAMEPAD_AXIS_LEFT_Y        1
#define SE_GAMEPAD_AXIS_RIGHT_X       2
#define SE_GAMEPAD_AXIS_RIGHT_Y       3
#define SE_GAMEPAD_AXIS_LEFT_TRIGGER  4
#define SE_GAMEPAD_AXIS_RIGHT_TRIGGER 5
#define SE_GAMEPAD_AXIS_LAST          SE_GAMEPAD_AXIS_RIGHT_TRIGGER